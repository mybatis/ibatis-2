...
public interface ResultObjectFactory {
   ...
   * <li>When processing a nested select - to create instances of parameter objects on the nested select</li>
   * <li>When processing result maps with nested result maps. iBATIS will ask the factory to create an instance of the
   * nested object. If the nested object is some implementation of <code>java.util.Collection</code> then iBATIS will
   * supply default implementations of the common interfaces if the factory chooses not to create the object. If the
   * embedded object is a <code>java.util.List</code> or <code>java.util.Collection</code> the default behavior is to
   * create an <code>java.util.ArrayList</code>. If the embedded object is a <code>java.util.Set</code> the default
   * behavior is to create a <code>java.util.HashSet</code> .</li>
   * </ul>
   * If you return <code>null</code> from this method, iBATIS will attempt to create in instance of the class with it's
   * normal mechanism. This means that you can selectively choose which objects to create with this interface. In the
   * event that you choose not to create an object, iBATIS will translate some common interfaces to their common
   ...
   *
   * @return a new instance of the specified class. The instance must be castable to the specified class. If you return
   *         <code>null</code>, iBATIS will attempt to create the instance using it's normal mechanism.
   *
